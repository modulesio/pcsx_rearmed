...
#ifdef PSXREC
  printf("use interpreter %d\n", Config.Cpu == CPU_INTERPRETER); // XXX
	if (Config.Cpu == CPU_INTERPRETER) {
		psxCpu = &psxInt;
	} else psxCpu = &psxRec;
#else
  printf("use interpreter by default\n"); // XXX
	psxCpu = &psxInt;
#endif
...
{
  printf("set PGXP mode %x %x %x %x\n", psxCpu, &psxInt, psxCpu->SetPGXPMode, pgxpMode); // XXX
